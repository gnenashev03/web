...
public class TaskService implements TaskServiceInterface {
    ...
    private void checkActiveLimit(Task task) {
        if (task.getCreatedBy() == null) {
            throw new IllegalStateException("Task must have createdBy set");
        }
    ...
    @Override
    public TaskDto update(Task iTask) throws TaskNotFoundException {
        Task existingTask = getTaskOrExcept(iTask.getId());
        existingTask.setTitle(iTask.getTitle());
        existingTask.setStatus(iTask.getStatus());
        checkActiveLimit(existingTask);
